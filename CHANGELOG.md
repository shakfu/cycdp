...
- The thread-context leak test measured process RSS, which is a proxy the allocator swamps. It passed on macOS and on ubuntu-latest and failed inside cibuildwheel's manylinux container, where glibc's per-thread arenas and stack cache grew 87 MB over 2,000 short-lived threads -- against the 1 MB that leaking every context would produce, so the signal sat two orders of magnitude under the noise. It now counts live contexts directly, through a new atomic counter (`cdp_lib_live_thread_contexts`, exposed for tests as `cycdp._core._live_thread_contexts`), and asserts the count returns to its starting value. That also makes the assertions sharper than RSS ever allowed: exactly one context is created on first use, eight concurrent threads hold exactly eight, and `release_thread_context()` drops exactly one and is idempotent.
